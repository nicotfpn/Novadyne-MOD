[Início](../README.md) · [Receitas](../receitas.md) · [Máquinas](../maquinas.md) · [Materiais](../materiais.md) · [Valves](../valves.md) · [Progressão](../progressao.md) · [Como testar](../testar.md)

---

# Valve (Tier 1)

![Valve (Tier 1)](../assets/generated/icon_valve_tier_1.png)

`novadyne:valve_tier_1`

**Upgrade da Lithography.** Tier 1: 70% de sucesso e 30% de falha na gravação. A valve não é consumida.

## Como obter

### Valve (Tier 1) — valve_tier_1

![Grade ou processo para Valve (Tier 1)](../assets/generated/valve_tier_1.png)

| Quantidade | Ingrediente |
| ---: | --- |
| 3 | Barra de ferro |
| 1 | Barra de cobre |

**Resultado:** 1 × [Valve (Tier 1)](../itens/valve_tier_1.md).

**Sem posição fixa:** basta colocar esses ingredientes na bancada, em qualquer ordem.

[Ver JSON da receita](../../src/main/resources/data/novadyne/recipe/valve_tier_1.json)

## Onde usar

- Craft de [Valve (Tier 2)](../itens/valve_tier_2.md).
- Slot de valve da [Lithography](litografia.md); melhora a chance de sucesso.

## Teste em criativo

```mcfunction
/give @s novadyne:valve_tier_1
```
